...
function GetUpType($type='')
{
	...
	global $cfYYS_UPLOADS_img_type,
		   $cfYYS_UPLOADS_soft_type,
		   $cfYYS_UPLOADS_media_type;


	if($type == 'image')
	{
		$uptype = explode('|',$cfYYS_UPLOADS_img_type);
		$upstr  = '';
		foreach($uptype as $v)
		{
			if(!empty($v))
	...
	{
		$uptype = explode('|',$cfYYS_UPLOADS_soft_type);
		$upstr  = '';
		foreach($uptype as $v)
		{
			if(!empty($v))
	...
	{
		$uptype = explode('|',$cfYYS_UPLOADS_media_type);
		$upstr  = '';
		foreach($uptype as $v)
		{
			if(!empty($v))
	...
	{
		$alltype = $cfYYS_UPLOADS_img_type.'|'.$cfYYS_UPLOADS_soft_type.'|'.$cfYYS_UPLOADS_media_type;
...
function SetImgContent(data)
{
	if(data == '')
	{
		alert('您上传的文件可能超过php.ini设置的最大内容提交大小[<?php echo get_yys_var('post_max_size'); ?>]！');
		return;
	}
	else
	{
...
<div class="W">
	...
	<div class="Wrap">
		<div class="Title">
			<h3 class="MainTit"><?php echo $biaoti; ?></h3>
