$(function() {
    ...
    var e = function() {
        ...
        if (_RID > 0) {
            var g = function() {
                return escape("http://www.1yyg.com/api/referrals.ashx?rid=" + _RID + "&url=" + escape("http://weixin.1yyg.com?s=share-u" + _RID))
            };
            var f = function(j) {
                try {
                    if (j.urls[0].result) {
                        var i = "d1元就能买iPhone 6S，一种很有意思的购物方式，快来看看吧！";
                        var l = j.urls[0].url_short.replace("t.cn", "t.1yyg.com");
                        $("#txtInfo").html(i + l)
                    }
                } catch(k) {
            ...
            GetJPData("http://info.1yyg.com", "getshorturl", "url=" + escape("https://api.weibo.com/2/short_url/shorten.json?source=1681459862&url_long=" + g()), f)
        }
        var h = $("#hidLineLink").val();
        wxShareFun({
            shareTitle: "e1元就能买iPhone 6S，一种很有意思的购物方式，快来看看吧！",
        ...
        $("#liRechagre").click(function() {
            if (parseInt(f) > 0) {
                ...
                $("#btnOK").click(function() {
                    GetJPData("http://weixin.1yyg.com", "memberCenterApplyToAccount", "",
                    function(l) {
                        if (l.code == 0) {
                            f = f - l.money;
                            $("div.f-pop-balance").remove();
