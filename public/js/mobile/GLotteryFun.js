...
function gg_show_time_add_li(div, path, info) {
    ...
    html += '<a href="' + path + '/goods/items/goodsId/' + info.id + '" target="_blank" class="u-lott-pic">';
    html += '<img src="' + publics + '/' + info.thumb + '">';
    html += '</a>';
    html += '<span class="countdown_div" style="line-height: 20px;background: #f60;">';

    html += '<em class="wenzi" style="color:#fff">倒计时:</em>';
    html += '<em class="shi" style="color:#fff"><em class="minute">99</em>:<em class="second">99</em>:<em class="millisecond">99</em></em>';
    html += '</span>';
    html += '</span>';
    html += '</li>';

    var uhtml = '';
    uhtml += '<a href="' + path + '/goods/items/goodsId/' + info.id + '" target="_blank" class="u-lott-pic">';
    uhtml += '<img src="' + publics + '/' + info.thumb + '">';
    uhtml += '</a>';

    uhtml += '<span class="winner">';
    uhtml += '恭喜<a class="blue z-user" href="' + path + '/mobile/userindex/id/' + ( parseInt(info.uid)) + '" target="_blank">';
    uhtml += info.user;
    uhtml += '</a>获得';
    uhtml += '</span>';
