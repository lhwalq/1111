$(function() {
    var a = function() {	 
        ...
        var r = function() {
            ...
            var w = function() {
                return "/FIdx/" + v.FIdx + "/" + v.EIdx + "/" + v.isCount
            };
            var y = function() {
                d.show();				 
               // GetJPData("http://m.1yyg.com", "getUserRecharge", w(),
				 $.getJSON(roots+"/user/getUserRecharge1"+w(),
                function(D) {
                    if (D.code == 0) {
                        var B = "";
                        if (v.isCount == 1) {
                            x = D.count;
                            v.isCount = 0;
                            B = '<li class="m-userMoneylst-tt"><span>兑换时间</span><span>兑换金额</span><span>兑换渠道</span></li>'
                        }
                        var C = D.listItems;
                        var A = C.length;
                        for (var z = 0; z < A; z++) {
        ...
        var t = function() {
            ...
            var y = function() {
                d.show();				
                $.getJSON(roots+"/user/getUserConsumption1"+w(),
                function(D) {
                    if (D.code == 0) {
                        var B = "";
                        if (v.isCount == 1) {
                            v.isCount = 0;
                            x = D.count;							 
                            B = '<li class="m-userMoneylst-tt"><span>申请时间</span><span>提现信息</span></li>'
                        }
                        var C = D.listItems;
                        var A = C.length;
                        for (var z = 0; z < A; z++) {
                            B += '<li class="' + (z + 1 == A ? "bornone": "") + '"><span>' + C[z].time + "</span><span>￥" + C[z].money + C[z].username + C[z].bankname + "</span></li>"
                        }
                        if (v.FIdx > 0) {
