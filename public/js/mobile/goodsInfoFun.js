$(function() {
	...
	var xx = $("#xiangou").val();
	var cc = $("#counts").val();
	var ss = $("#xiangouss").val();
	var deng = $("#denglu").val();
	
    var m = $("#hidShowTime").val() == "1";
    if (m) {
        $("#divPic").addClass("pPicBor");
        Base.getScript(pub+"/js/GoodsTimeFun.js",
        function() {
        ...
        var l = function(r) {
		if (deng == null || deng == undefined || deng == ''&&xx == 1) { 
d("限购商品,请先登陆")
	location.href = Gobal.Webpath+"/mobile/user/login"
} else{

		if(xx == 1 && parseInt(cc) > parseInt(ss))
			{
				d('限购5人次哦')
			}else
					{
        ...
        Base.getScript(pub+"/js/mobile/pageDialog.js", i);
        var g = $("#divAutoRTime");
        if (g.length > 0) {
            var o = g.attr("time");
            if (o) {
                ...
                Base.getScript(pub+"/js/mobile/CountdownFun.js",
                function() {
                    g.countdowntime(e, k)
                });
				//获取当前商品
    ...
    var a = function() {
        ...
        Base.getScript(pub+"/js/mobile/GoodsPicSlider.js?v=130826",
        function() {
            $("#sliderBox").picslider()
        })
    };
    Base.getScript(pub+"/js/mobile/PeriodSlider.js?v=130826", a)
});