$(function () {
	...
	var e = function () {
		var h = function () {
			var i = function () {
				return "/" + a.FIdx + "/" + a.EIdx + "/" + a.isCount + "/" + a.state
			};
			var j = function () {
				//GetJPData("", "getUserBuyList", i()
				$.getJSON("/mobile"+"/shopajax"+"/getUserBuyListNew"+i()
				, function (q) {
					if (q.code == 0) {
						if (a.isCount == 1) {
							b = q.count;
						...
						for (var r = 0; r < s; r++) {
							...
							if (m == 1) {
								...
								n += '<a href="javascript:;">';
								n += '<cite><img src="' + Gobal.loadpic + '" src2="/love/uploads/' + k.goodsPic + '" /><i>进行中</i>' + (p[r].codeType == 3 ? '<div class="pPurchase">限购</div>' : "") + "</cite>";
								n += "<dl>";
								...
								if (m == 2) {
									n += '<li codeid="' + k.codeid + '" class="in_progress">';
									n += '<a href="/mobile/user/buyDetail/' + k.codeid + '">';
									n += '<cite><img src="' + Gobal.loadpic + '" src2="/love/uploads/' + k.goodsPic + '" /><i>已满员</i>' + (p[r].codeType == 3 ? '<div class="pPurchase">限购</div>' : "") + "</cite>";
									n += "<dl>";
									n += "<dt>(第" + k.codeperiod + "云)" + k.goodsname + "</dt>";
									n += '<dd>已参与<em class="orange">' + k.buynum + "</em>人次</dd>";
									...
									if (m == 3) {
										...
										n += '<cite><a href="/mobile/user/buyDetail/' + k.codeid + '"><img src="' + Gobal.loadpic + '" src2="/love/uploads/' + k.goodsPic + '" /></a><i>已揭晓</i>' + (p[r].codeType == 3 ? '<div class="pPurchase">限购</div>' : "") + "</cite>";
										n += "<dl>";
										n += '<dt><a href="/mobile/user/buyDetail/' + k.codeid + '" class="gray6">(第' + k.codeperiod + "云)" + k.goodsname + "</a></dt>";
										n += '<dd>已参与<em class="orange">' + k.buynum + "</em>人次</dd>";
										n += '<dd>获得者：<a href="' + roots + '/mobile/userindex/' + k.q_uid + '" class="blue">' + k.username + "</a></dd>";
							...
							o.bind("click", function () {
								location.href = "/mobile/user/buyDetail/" + $(this).attr("codeid") + ""
