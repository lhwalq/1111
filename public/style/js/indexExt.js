var formatDate = function (date, format) {
    ...
    var dict = {"yyyy": date.getFullYear(), "M": date.getMonth() + 1, "d": date.getDate(), "H": date.getHours(), "m": date.getMinutes(), "s": date.getSeconds(), "S": date.getMilliseconds(), "MM": ("" + (date.getMonth() + 101)).substr(1), "dd": ("" + (date.getDate() + 100)).substr(1), "HH": ("" + (date.getHours() + 100)).substr(1), "mm": ("" + (date.getMinutes() + 100)).substr(1), "ss": ("" + (date.getSeconds() + 100)).substr(1)};
...
function lazyload200(id) {
    ...
    $("img.lazy" + id).lazyload({
        effect: "fadeIn",
        placeholder: pub + "/style/images/loading_200.gif",
        threshold: 100,
...
var addClick = function () {
    $(".yContent").delegate(".w_goods_three,.w_imgOut,.w_slip", "click", function (ev) {
        ...
        window.open(roots + '/goods/items/goodsId/' + gid + '.html');
    });
    ...
    if (cart == null || cart == '' || cart == "undefined") {
        ...
        if (list.length >= 30) {
            ...
            if (check == 0) {
                ...
                //var newnumber=ordernumber+number;//未实现
