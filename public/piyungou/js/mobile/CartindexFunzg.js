    ...
    var b = function() {
        ...
        if (h.length > 0) {
            h.children("a").click(function() {
                location.href = Gobal.Webpath+"/mobile/cart/payzg"   //付款页面
            })
        }
        ...
        var m = function() {
            ...
            $("input:text[name=num]", a).each(function(s) {
				...
				var yunjiage = parseInt($(this).attr("yunjiage"));
                if (!isNaN(t)) {
                    r++;
                    q += t*yunjiage
                }
            ...
            if (r > 0) {
                ...
                g.children("p").html('总共一元潮购<span class="orange arial z-user">' + r + '</span>个商品  合计金额：<span class="orange arial">' + q + ".00</span> 元")
            } else {
                g.remove()
            }
        };
        var d = function() {
            ...
            var u;
            o = t;
			var ss = $("#countss" +v).val();
			var yunjiages = document.getElementById('txtNum'+v).value ;
			var xg = $("#xg" +s).val();
        ...
        var p = function(r, u) {
            ...
            if (r > 100) {
                if (q.length == 0) {
                    var s = $('<div class="z-Cart-tips">已超过100人次，一元潮购存在一定风险，请谨慎参与！</div>');
                    t.prepend(s)
                }
            } else {
                q.remove()
            ...
            var s = function(w) {
                ...
                if (w.code == 1) {
                    ...
                    e("本期商品已一元潮购光了", q, -75, u)
						...
						var yunjiage = parseInt(q.attr("yunjiage"));
						
                        q.parent().prev().html('总共一元潮购：<em class="arial">' + r + '</em>人次/<em class="orange arial">￥' + r*yunjiage + ".00</em>")
                    }
                }
            };
            GetJPData(Gobal.Webpath, "ajax", "addShopCartzg/" + t + "/" + r+"/cart", s)
		 //  $.getJSON("/mobile/ajax/"+"addShopCart", s)
        };
        var k = function(w, v) {
        ...
        $("a[name=delLink]", a).each(function(q) {
            ...
            function() {			 
                ...
                var s = function() {
                    ...
                    GetJPData(Gobal.Webpath, "ajax", "delCartItemzg/" + t, u)
                };
                n("您确定要删除吗？", s)
            })
        })
