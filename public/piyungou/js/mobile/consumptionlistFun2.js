$(function() {
    var a = function() {	 
        ...
        var r = function() {
            ...
            var y = function() {
               ...
               // GetJPData("http://m.1yyg.com", "getUserRecharge", w(),
				 $.getJSON("/mobile"+"/shopajax"+"/getUserRecharge2"+w(),
                function(D) {
                    if (D.code == 0) {
                        var B = "";
                        if (v.isCount == 1) {
                            x = D.count;
                            v.isCount = 0;
                            B = '<li class="m-userMoneylst-tt"><span>获得时间</span><span>福分金额</span><span>福分渠道</span></li>'
                        }
                        var C = D.listItems;
                        var A = C.length;
                        for (var z = 0; z < A; z++) {
        ...
        var t = function() {
            ...
            var y = function() {
                d.show();				
                $.getJSON("/mobile"+"/shopajax"+"/getUserConsumption2"+w(),
                function(D) {
                    if (D.code == 0) {
                        var B = "";
                        if (v.isCount == 1) {
                            v.isCount = 0;
                            x = D.count;							 
                            B = '<li class="m-userMoneylst-tt"><span>消费时间</span><span>消费信息</span></li>'
                        }
                        var C = D.listItems;
                        var A = C.length;
                        for (var z = 0; z < A; z++) {
                            B += '<li class="' + (z + 1 == A ? "bornone": "") + '"><span>' + C[z].time + "</span><span>￥" + C[z].money +  "福分</span></li>"
                        }
                        if (v.FIdx > 0) {
