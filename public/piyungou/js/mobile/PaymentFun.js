    ...
    var b = function () {
        ...
        var u = function () {
            ...
            if (w + g >= x) {
                ...
                if (shopnum != -1) {
                    if (shopnum == 0) {
                        //3/15改用表单提交方式  加入选购功能 //location.replace(Gobal.Webpath+"/mobile/cart/paysubmit/"+checkpay+"/"+banktype+"/"+x+"/"+t)
                        var chooselist = localStorage.getItem("chooselist");
                        chooselist = chooselist ? JSON.parse(chooselist) : null;
                        if ($("#form_paysubmit").length <= 0) {
                            var url = Gobal.Webpath + "/mobile/cart/paysubmit/" + checkpay + "/" + banktype + "/" + x + "/" + t;
                            var str = "";
