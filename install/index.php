...
//参数设置
define('SOFT_VERSION', 'yyygcms');
define('SOFT_RELEASE', $version);
define('ROOT_PATH', dirname(__FILE__).'/../');//网站根目录
define('SOFT_NAME', '一元杀yyygcms一元云购源码安装'.SOFT_VERSION);
...
'license'=>'<div class="license"><h1>系统安装协议</h1>
                <p>版权所有 (c)2012-2017 保留所有权利。 </p>
				<p>感谢您选择yyygcms基于 PHP + MySQL的技术开发，全部源码开放。</p>
			...
			<strong>二、协议许可的权利 </strong>
				...
				<p>2、您可以在协议规定的约束和限制范围内修改yyygcms源代码或界面风格以适应您的网站要求。 </p>
			...
			<strong>三、协议规定的约束和限制 </strong>
				...
				<p>3、不管你的网站是否整体使用yyygcms，还是部份栏目使用 ，在你使用了yyygcms的网站主页上必须加上yyygcms.cn。</p>
				<p>4、未经乐清市一元杀网络有限公司官方许可，禁止在yyygcms的整体或任何部分基础上以发展任何派生版本、修改版本或第三方版本用于重新分发。</p>
			...
			<strong>四、有限担保和免责声明 </strong>
				...
				<p>3、电子文本形式的授权协议如同双方书面签署的协议一样，具有完全的和等同的法律效力。您一旦开始确认本协议并安装   ，即被视为完全理解并接受本协议的各项条款，在享有上述条款授予的权力的同时，受到相关的约束和限制。协议许可范围以外的行为，将直接违反本授权协议并构成侵权，我们有权随时终止授权，责令停止损害，并保留追究相关责任的权力。</p>
				<p>4、如果本软件带有其它软件的整合API示范例子包，这些文件版权不属于本软件官方，并且这些文件是没经过授权发布的，请参考相关软件的使用许可合法的使用。</p>
				<p><b>版本最新更新：</b> '.SOFT_RELEASE.'</p>
                <p align=right>BY 乐清市一元杀网络有限公司</p>
</div>',
'agreement_yes' => '我同意，继续安装',
'agreement_no' => '我不同意',
'title_install' => ' 一元杀一元云购yyygcms'.SOFT_VERSION.' 安装程序',
...
'tips_dbinfo' => '数据库信息',
'dbhost' => '数据库服务器',
'dbhost_comment' => '如出现错误，此处换为localhost',
'dbuser' => '数据库用户名',
'dbpw' => '数据库密码',
'dbname' => '数据库名',
'tablepre' => '数据表前缀',
...
'password' => '管理员密码',
'password_comment' => '安装成功后，进后台修改',
'password2' => '重复密码',
'password2_comment' => '密码必须为:yyygcms.cn',
...
'install_succ_remark'=>'<p class=\'nest\'>系统安装完成：</p>
		<p class=\'nes\'>1.系统后台地址：<a href=\'http://'.$_SERVER['HTTP_HOST'].trim(str_replace(strstr($_SERVER['SCRIPT_NAME'], '/install'),"",$_SERVER['SCRIPT_NAME'])).'/admin\' target=\'_blank\'>http://'.$_SERVER['HTTP_HOST'].trim(str_replace(strstr($_SERVER['SCRIPT_NAME'], '/install'),"",$_SERVER['SCRIPT_NAME'])).'/admin</a></p>
...
'php_version_too_low'=>'PHP版本过低，最低版本要求5.3或以上版本',
...
$env_items = array(
'php' => array('c' => 'PHP_VERSION', 'r' => '5.3', 'b' => '5.3'),
'gdversion' => array('r' => '1.0', 'b' => '2.0'),
'diskspace' => array('r' => '2G', 'b' => 'notset'),
);
$dirfile_items = array(
    'conf' => array('type' => 'file', 'path' => './Common/Conf/config.php'),
	
   'c' => array('type' => 'dir', 'path' => './Common/Conf/'),
...
$form_db_init_items = array(
	...
	'admininfo' => array(
		...
		'password2' => array('type' => 'text', 'required' => 1, 'reg' => '/^.*$/', 'value' => array('type' => 'var', 'var' => 'password2')),
	)
);
...
if (file_exists('../yyygcms/install.lock')){
	@header("Content-type: text/html; charset=UTF-8");
	echo "系统已经安装过了，如果要重新安装，那么请删除yyygcms目录下的install.lock文件";
	exit;
}
...
if($method == 'show_license') {//声明
	...
	if($submit) {
		...
		if($confstatus['status']==1){
			...
			$configfile = 'sql/config.php';
			//echo iconv('GB2312','UTF-8',file_get_contents($configfile));
			
	        $strConfig=@file_get_contents($configfile);
            $strConfig = str_replace('#DB_HOST#', $dbhost, $strConfig);
            $strConfig = str_replace('#DB_NAME#', $dbname, $strConfig);
            $strConfig = str_replace('#DB_USER#', $dbuser, $strConfig);
            $strConfig = str_replace('#DB_PWD#', $dbpw, $strConfig);
            $strConfig = str_replace('#DB_PORT#', $dbport, $strConfig);
            //$strConfig = str_replace('#DB_PREFIX#', $dbPrefix, $strConfig);
			
            $re = @file_put_contents('../Common/Conf/config.php', $strConfig);
			$sqlfile = 'sql/yyygcms.sql';
			...
			$fp = @fopen('../yyygcms/install.lock','wb+');
			@fclose($fp);
		}else{
	...
	$username = 'admin';
	$password = 'yyygcms.cn';
	$password2 = 'yyygcms.cn';
	$fanli_name = '一元杀一元云购yyygcms';
...
<meta content="一元杀一元云购yyygcms" name="Copyright" />
...
?>
			<div class="footer">
			    <p>Powered by 一元杀一元云购(c) V4.5 2012-2017</p>
				<p>版权所有</p>
			</div>
		</div>
		</div>
...
function config_edit() {
	...
	$configfile['DB_PREFIX']=$tablepre;
		$configfile['DB_BACKUP']='backup/database/';
		$configfile['DB_PARAMS']='array(\PDO::ATTR_CASE => \PDO::CASE_NATURAL)';
		$configfile['DEFAULT_CONTROLLER']='Index';
		$configfile['DEFAULT_Action']='index';
		$configfile['URL_HTML_SUFFIX']='html';
		$configfile['SESSION_AUTO_START']='false';
	       $configfile['DEFAULT_MODULE']='Home';
