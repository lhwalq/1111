...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        ...
        <div class="table_form lr10">
            <form name="form" action="/brand/save" method="post">
                <table width="100%"  cellspacing="0" cellpadding="0">
                    <tr>
                        <td align="right" class="wid100">所属栏目：</td>
                        <td>
                            <div id="category_select" style="float:left">	
                                <?php  foreach($cateid_arr as $cateid){ ?>
                                <span><input name='cateid[]' value='<?php echo ($cateid); ?>'/><?php echo R('admin/huode_lanmu',array($cateid));?><b>&nbsp;×</b></span>&nbsp;";
                                <?php
 } ?>
                            </div>
                            <?php if($pinpais): ?><input type="hidden" name='id' value='<?php echo ($pinpais["id"]); ?>'/>
                                <a style="color:#f00;float:left;"  class="pay_window_show"> 【继续添加所属栏目】 </a> 
                                <?php else: ?>
                                <a style="color:#f00;float:left;"  class="pay_window_show"> 【添加所属栏目】 </a><?php endif; ?>                            
                        </td>
                    </tr>
                    <tr>
                        <td align="right">品牌名称：</td>
                        <td><input type="text"  name="name" class="input-text wid100" value="<?php echo $pinpais['name'] ; ?>"></td>
                    </tr>
                    <tr>
                        <td align="right">排序：</td>
                        <td><input type="text"  name="order" onKeyUp="value = value.replace(/[^\d]/ig, '')" class="input-text wid100" value="<?php echo $pinpais['order'] ; ?>">
