...
<div class="W">
	...
	<div class="Wrap" id="Wrap">
		<div class="Title">
			<h3 class="MainTit" id="MainTit"><?php echo $biaoti; ?></h3>
...
<script src="/Public/plugin/uploadify/jquery.min.js" type="text/javascript"></script> 
<script src="/Public/plugin/uploadify/jquery.uploadify.js" type="text/javascript"></script> 
<script src="/Public/plugin/uploadify/uploadify-move.js" type="text/javascript"></script> 
...
$(function() {
	$('#uploadify').uploadify({
			'formData'        : {
				...
				'type'		  : '<?php echo R("base/encrypt",array($type));?>',
				'path'		  : '<?php echo R("base/encrypt",array($path));?>',
				'size'		  : '<?php echo R("base/encrypt",array($size));?>',
...
function SetImgContent(data){
	...
	if(obj.ok == 'no'){
		...
		var sLi = "";
		sLi += '<li class="img">';
		sLi += '<img src="/Public/uploads/' + obj.text + '" width="100" height="100" onerror="this.src=\'/Public/plugin/uploadify/nopic.png\'">';
...
$("#SaveBtn").click(function(){							 
	if(<?php echo $num; ?> > 1){
			...
			$("input[name^='fileurl_tmp']").each(function(){
				fileurl_tmp += '<li rel="'+ this.value +'"><input class="input-text" type="text" name="<?php echo $input;?>[]" value="'+ this.value +'" /><a href="javascript:void(0);" onclick="ClearPicArr(\''+ this.value +'\',\'<?php echo C("URL_DOMAIN");?>\')">删除</a></li>';	
			});
