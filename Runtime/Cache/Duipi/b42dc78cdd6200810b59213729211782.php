...
<!-- 导航   end  --> 
<link rel="stylesheet" type="text/css" href="/Public/style/css/pager.css"></link>
  <!-- 导航   end  --> 
  ...
  <div class="w_con"> 
   <h4 class="w_guide"><a href="/index/index"><?php echo (L("web_glist_first")); ?></a><a class="w_accord" href="/goods/glist"><?php echo (L("web_glist_all_goods")); ?></a></h4>
   <div class="w_goods_nav"> 
    <h2><?php echo (L("web_glist_all_goods")); ?><span>（<?php echo (L("web_shaidan_gong")); ?><a href="javascript:void%200"><?php echo ($zongji); ?></a><?php echo (L("web_glist_all_jian")); ?>）</span></h2>
    <div class="w_choose"> 
     <dl class="w_choose_list" >
     <?php  if($cid){ $two_cate_list = D("fenlei")->where(array("parentid"=>$cid))->order("`order` DESC")->field("cateid,parentid,name")->select(); if(!$two_cate_list && $daohangs['parentid']) $two_cate_list = D("fenlei")->where(array("parentid"=>$daohangs[parentid]))->order("`order` DESC")->field("cateid,parentid,name")->select(); }else{ $two_cate_list = D("fenlei")->where(array("parentid"=>0,"model"=>'1'))->order("`order` DESC")->field("cateid,parentid,name")->select(); } $two_cate_listda = D("fenlei")->where(array("parentid"=>0,"model"=>'1'))->order("`order` DESC")->field("cateid,parentid,name")->limit("7")->select(); $two_cate_listfenlei = D("fenlei")->where(array("parentid"=>0,"model"=>'1'))->order("`order` DESC")->field("cateid,parentid,name")->select(); $jjj='1'; ?>

     <?php if(is_array($two_cate_listda)): foreach($two_cate_listda as $key=>$two_cate): ?><dd>
      <?php  $urls=str_replace("_0_0", "", I("type")); ?>
       <a href="/goods/glist/type/<?php echo ($two_cate['cateid']); ?>_0_0.html" data-id="2" 
      <?php  $urls=str_replace("_0_0", "", I("type")); if($urls==$two_cate['cateid']){ echo "class='w_selected'"; } ?> ><span class="w_icon w_icon_<?php  if($jjj==1){echo one;} else if($jjj==2){echo two;} else if($jjj==3){echo three;} else if($jjj==4){echo four;} else if($jjj==5){echo five;} else if($jjj==6){echo six;} else if($jjj==7){echo seven;} else{echo eight;} $jjj++; ?>"></span><b><?php echo ($two_cate['name']); ?></b></a>
...
var height=$("#ddBrandList_brand").innerHeight();
	if(height==60){
		...
		$(".list_classMore").html("<?php echo (L("web_glist_all_pack_up")); ?><i></i>");
	}else{
		$("#ddBrandList_brand").css("height","60px");
		$(".list_classMore").removeClass("MoreClick");
		$(".list_classMore").html("<?php echo (L("web_glist_all_develop")); ?><i></i>");
	};
}
$(function(){	
	$(".list_classMore").click(showh);
	 ...
	 <dl class="w_goods_brand"> 
		...
		<dd  class="w_specific_class1">
         ...
         <ul>
<li>
<a class="w_effect" href="/goods/glist/type/type/<?php echo ($cid); ?>_0_0.html"><?php echo (L("web_glist_all")); ?></a>
</li>
<?php if(is_array($two_cate_list)): foreach($two_cate_list as $key=>$two_cate): if($cid == $two_cate['cateid'] ): ?><li><a href="/goods/glist/type/<?php echo ($two_cate['cateid']); ?>_0_0.html" class="w_selected" title="<?php echo ($two_cate['name']); ?>"><?php echo ($two_cate['name']); ?></a></li>
					<?php else: ?>
					<li><a href="/goods/glist/type/<?php echo ($two_cate['cateid']); ?>_0_0.html" title="<?php echo ($two_cate['name']); ?>"><?php echo ($two_cate['name']); ?></a></li><?php endif; endforeach; endif; ?>
       ...
       <dl class="w_goods_brand"> 
        <dd>
            <?php echo (L("web_glist_brand")); ?>
        </dd> 
        <?php if(count($pinpai) > 17): ?><dd id="ddBrandList_brand" class="w_specific_class1" style="height: 60px;">
		<?php else: ?>
		<dd id="ddBrandList_brand" class="w_specific_class1" ><?php endif; ?>
         <ul>
<li>
<a class="w_effect" href="/goods/glist/type/<?php echo ($cid); ?>_0_<?php echo ($order); ?>.html"><?php echo (L("web_glist_all")); ?></a>
</li>
  <?php if(is_array($pinpai)): foreach($pinpai as $key=>$pinpais): if($pinpais['id'] == $bid): ?><li>
<a  href="/goods/glist/type/<?php echo ($cid); ?>_<?php echo ($pinpais['id']); ?>_<?php echo ($order); ?>.html" class="w_selected"><?php echo ($pinpais['name']); ?></a>
</li>
<?php else: ?>
<li>
<a  href="/goods/glist/type/<?php echo ($cid); ?>_<?php echo ($pinpais['id']); ?>_<?php echo ($order); ?>.html"><?php echo ($pinpais['name']); ?></a>
</li><?php endif; endforeach; endif; ?>

</ul>
<?php if(count($pinpai) > 17): ?><a class="list_classMore" href="javascript:;"><?php echo (L("web_glist_all_develop")); ?><i></i></a><?php endif; ?>
        </dd> 
        <div class="w_clear"></div> 
       </dl> 
      </div> 
   ...
   <div class="w_product_con"> 
    <dl class="w_new"> 
     <dd>
         <?php echo (L("web_glist_arrange")); ?>：
     </dd> 
      <dd>
      <a href="/goods/glist/type/<?php echo ($cid); ?>_<?php echo ($bid); ?>_1.html"  <?php if($order == '1'): ?>class="w_announced"<?php endif; ?> data-id="publicTime"><?php echo (L("web_lottery_unveiled")); ?></a>
     </dd> 
     <dd>
      <a href="/goods/glist/type/<?php echo ($cid); ?>_<?php echo ($bid); ?>_3.html"  <?php if($order == '3'): ?>class="w_announced"<?php endif; ?>  data-id="takedout"><?php echo (L("web_lottery_residue")); ?></a>
     </dd> 
     <dd>
      <a href="/goods/glist/type/<?php echo ($cid); ?>_<?php echo ($bid); ?>_2.html"  <?php if($order == '2'): ?>class="w_announced"<?php endif; ?>  data-id="periods"><?php echo (L("web_glist_hot_sale")); ?></a>
     </dd> 
     <dd>
      <a href="/goods/glist/type/<?php echo ($cid); ?>_<?php echo ($bid); ?>_4.html"  <?php if($order == '4'): ?>class="w_announced"<?php endif; ?> data-id="addTime"><?php echo (L("web_glist_new_goods")); ?></a>
     </dd> 
     <dd>
      <?php if($order == '5'): ?><a class="w_last" href="/goods/glist/type/<?php echo ($cid); ?>_<?php echo ($bid); ?>_6.html" data-id="totalPrice_up"><?php echo (L("web_glist_price")); ?><span><img src="/Public/style/images/jia_03.png" /></span><span class="w_jian_tou"><img src="/Public/style/images/jia_06.png" /></span></a>
 <?php else: ?>
     <?php if($order == '6'): ?><a class="w_last" href="/goods/glist/type/<?php echo ($cid); ?>_<?php echo ($bid); ?>_5.html" data-id="totalPrice_up"><?php echo (L("web_glist_price")); ?><span><img src="/Public/style/images/jia_03.png" /></span><span class="w_jian_tou"><img src="/Public/style/images/jia_06.png" /></span></a>
      <?php else: ?>
            <a class="w_last" href="/goods/glist/type/<?php echo ($cid); ?>_<?php echo ($bid); ?>_5.html" data-id="totalPrice_up"><?php echo (L("web_glist_price")); ?><span><img src="/Public/style/images/jia_03.png" /></span><span class="w_jian_tou"><img src="/Public/style/images/jia_06.png" /></span></a><?php endif; endif; ?>
     </dd> 
    </dl> 
   </div> 
   <!--商品列表--> 
   ...
   <?php $ii = '0'; ?>
   <?php if(is_array($yyslist)): foreach($yyslist as $key=>$shop): ?><li class="w_goods_details ">
      <div class="w_imgOut" data-gid="<?php echo ($shop['id']); ?>" data-pid="<?php echo ($shop['zongrenshu']); ?>">
       <a data-gid="<?php echo ($shop['id']); ?>" data-pid="<?php echo ($shop['zongrenshu']); ?>" target="_blank" class="w_goods_img" href="/goods/items/goodsId/<?php echo ($shop['id']); ?>.html"><img style="display: inline;" src="/Public/uploads/<?php echo ($shop['thumb']); ?>" id="img_<?php  echo $ii; ?>" data-gid="<?php echo ($shop['id']); ?>" data-pid="<?php echo ($shop['zongrenshu']); ?>" class="lazy1" data-original="/Public/uploads/<?php echo ($shop['picarr']); ?>" />
        <noscript>
         <img src="/Public/uploads/<?php echo ($shop['picarr']); ?>" alt="" />
        </noscript></a>
      </div><a class="w_goods_three" target="_blank" href="/goods/items/goodsId/<?php echo ($shop['id']); ?>.html" data-gid="<?php echo ($shop['id']); ?>" data-pid="<?php echo ($shop['zongrenshu']); ?>" title="<?php echo ($shop['title']); ?>">(<?php echo (L("web_user_di")); echo ($shop['qishu']); echo (L("web_user_qi")); ?>)<?php echo ($shop['title']); ?></a><b><?php echo (L("web_glist_value")); ?>：<?php echo (L("web_lottery_mark")); echo ($shop['money']); ?></b>
      <div class="w_line">
       <span style="width:<?php echo R("base/width",array($shop['canyurenshu'],$shop['zongrenshu'],244));?>px;"></span>
      </div>
      <ul class="w_number">
       <li class="w_amount"><?php echo ($shop['canyurenshu']); ?></li>
       <input id="cart_gid_0" value="<?php echo ($shop['id']); ?>" type="hidden" />
       ...
       <input id="title_0" value="<?php echo ($shop['title']); ?>" type="hidden" />
       ...
       <li><?php echo (L("web_glist_all_person")); ?></li>
       <li><?php echo (L("web_lottery_residue")); ?></li>
      </ul>
      <dl class="w_rob">
       <dd>
        <a class="w_slip" target="_blank" href="/goods/items/goodsId/<?php echo ($shop['id']); ?>.html" data-gid="<?php echo ($shop['id']); ?>" data-pid="<?php echo ($shop['zongrenshu']); ?>"><?php echo (L("web_glist_immediately")); ?></a>
       </dd>
       <dd class="w_rob_out">
        <a class="w_rob_in" href="javascript:void(0);" onclick="cartoong(<?php  echo $shop['id'].",".$ii;$ii++; ?>)"><?php echo (L("web_glist_gouwudai")); ?></a>
       </dd>
      </dl>
