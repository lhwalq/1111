...
<form action="" method="post">
 时间搜索: <input name="posttime1" type="text" id="posttime1" class="input-text posttime"  readonly="readonly" value="<?php echo !empty($posttime1)?date("Y-m-d H:i:s",$posttime1):''?>"/> -  
 		  <input name="posttime2" type="text" id="posttime2" class="input-text posttime"  readonly="readonly" value="<?php echo !empty($posttime2)?date("Y-m-d H:i:s",$posttime2):''?>"/>
			...
			<?php  if(isset($chongzhi)){ echo ($chongzhi=='请选择充值来源')?'selected':''; } ?>
			>请选择充值来源</option>
			<option value="充值" 
			<?php  if(isset($chongzhi)){ echo ($chongzhi=='充值')?'selected':''; } ?>
			>通过网络充值</option>
			<option value="使用佣金充值到一元云购账户"
			<?php  if(isset($chongzhi)){ echo ($chongzhi=='使用佣金充值到一元云购账户')?'selected':''; } ?>
			>使用佣金充值到一元云购账户</option>
			<option value="使用余额宝充值到一元云购账户" 
			<?php  if(isset($chongzhi)){ echo ($chongzhi=='使用余额宝充值到一元云购账户')?'selected':''; } ?>
			>使用余额宝充值到一元云购账户</option>
			</select>
			<select name="yonghu">
			<option value="请选择用户类型" 
			<?php  if(isset($yonghu)){ echo ($yonghu=='请选择用户类型')?'selected':''; } ?>
			>请选择用户类型</option>
			<option value="用户id" 
			<?php  if(isset($yonghu)){ echo ($yonghu=='用户id')?'selected':''; } ?>
			>用户id</option>
			<option value="用户名称" 
			<?php  if(isset($yonghu)){ echo ($yonghu=='用户名称')?'selected':''; } ?>
			>用户名称</option>
			<option value="用户邮箱" 
			<?php  if(isset($yonghu)){ echo ($yonghu=='用户邮箱')?'selected':''; } ?>
			>用户邮箱</option>
			<option value="用户手机" 
			<?php  if(isset($yonghu)){ echo ($yonghu=='用户手机')?'selected':''; } ?>
			>用户手机</option>
			</select>
...
<!--start-->
  <table width="100%" cellspacing="0">
    ...
    <tbody>
    	<?php  for($j=0;$j<count($recharge);$j++){ ?>
		<tr>
			<td align="center">
				<?php  if($recharge[$j]['username'])echo $recharge[$j]['username'];else if($recharge[$j]['mobile'])echo $recharge[$j]['mobile'];else {echo $recharge[$j]['email'];}?>
			</td>
