...
<div class="R-content">
	...
	<div class="last">
		<ul>
			<li><h3>您的账户可用余额为：</h3><b class="orange"><?php echo ($huiyuan['money']); ?></b>元 <a href="/user/userrecharge" class="bluebut" title="充值">充值</a>

			</li>
		<b class="red" style="float:left; margin-top:-40px; margin-left:12px;">
<br>
如果您的银行卡已被扣款，但平台显示未付款，请在七个工作日内联系客服<?php echo C('cell');?>
</b>
		</ul>
...
<ul class="listTitle">
		...
		<li class="code">备注</li>
		<li class="price">来源/状态</li>
		<li class="regard">交易金额</li>
	
	</ul>
 
        <?php if(is_array($$account1)): foreach($$account1 as $key=>$at1): ?><ul>
                
			<li class="leftTitle fontAri"><?php echo (date($at1['time'],"Y-m-d")); ?></li>
			<li class="code"><?php echo ($at1['trans_no']); ?></li>
			<li class="price">
			{if $at1['trans_type'] == 1}
                        <?php if(($at1['trans_type'] == 1)): ?>晋商贷用户购物卡
			<?php else: ?>
				晋商贷用户转账<?php endif; ?>		
			</li>
			<li class="regard">
				<font color="#0c0">￥<?php echo ($at1['trans_money']); ?></font>
			</li>

		</ul><?php endforeach; endif; ?>
	<hr>
	
        <?php if(is_array($account)): foreach($account as $key=>$at): ?><ul>
		<li class="leftTitle fontAri"><?php echo (date("Y-m-d",$at['time'])); ?></li>
		<li class="code"><?php echo ($at['content']); ?></li>
                <li class="price"><?php echo ($at['pay_type']); ?>
			
                        
                        <?php if(($at['status'] == '未付款')): ?>未付款
			<?php else: ?>
				已付款<?php endif; ?>	
		</li>
		<li class="regard">
		...
		<div id="divPageNav" class="page_nav">
        	<?php echo $fenye->show('one');?> <li>云购充值共 <?php echo ($zongji); ?> 条</li>
        </div>
