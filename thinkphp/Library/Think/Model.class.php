...
class Model {
     ...
     *演示代码
        $model = new \Think\Model();
          ...
          var_dump($model->execute_sp("sp_test",$params));
            exit;
     * 或者 需要返回值的
     * 
     * 则调用  var_dump($model->execute_sp("sp_test",$params,array("type"=>"string")));
     * 
     *  测试用存储过程
     ...
     * 
     * $return_param 为返回值参数 
     * 默认返回 output参数，但是若有return参数，则返回存储过程return值
    *      */
    public function execute_sp($sp_name,$params,$return_param=array()) {
        return $this->db->execute_sp($sp_name,$params,$return_param); 
    }
