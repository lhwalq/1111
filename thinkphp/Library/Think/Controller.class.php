...
$this->display("public/404");
 //404状态码 非法操作
    ...
    //}
protected function error($string = null, $defurl = null, $type = 0, $time = 2, $config = null) {
        if (empty($defurl)) {
        ...
        $this->display("public/smobile.error");
        exit;
    }
    /**
     * 操作成功跳转的快捷方法
     ...
     * @return void
     */
  //  protected function success1($message = '', $jumpUrl = '', $ajax = false) {
       // $this->dispatchJump($message, 1, $jumpUrl, $ajax);
   // }
   protected function success($string = null, $defurl = null, $type = 0, $time = 2, $config = null) {
        if (empty($defurl)) {
            $defurl = isset($_SERVER["HTTP_REFERER"]) ? $_SERVER["HTTP_REFERER"] : '';
            if (empty($defurl))
                $defurl = dirname(isset($_SERVER['SERVER_PORT']) && $_SERVER['SERVER_PORT'] == '443' ? 'https://' : 'http://' . (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : '') . $_SERVER['SCRIPT_NAME']);
