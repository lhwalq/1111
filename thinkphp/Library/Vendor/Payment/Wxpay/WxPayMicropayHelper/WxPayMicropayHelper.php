...
class Common_util_micropay
{
	...
	public function getSign($Obj)
	{
		...
		//签名步骤二：在string后加入KEY
		$String = $String."&key=".WxPayConf_micropay::KEY;
		//echo "【string】 =".$String."</br>";
		//签名步骤三：MD5加密
	...
	function postXmlSSLCurl($xml,$url,$second=30)
	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT, WxPayConf_micropay::SSLCERT_PATH);
		//默认格式为PEM，可以注释
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY, WxPayConf_micropay::SSLKEY_PATH);
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
...
class Wxpay_client_micropay extends Common_util_micropay
{
	...
	function createXml()
	{
	   	$this->parameters["appid"] = WxPayConf_micropay::APPID;//公众账号ID
	   	$this->parameters["mch_id"] = WxPayConf_micropay::MCHID;//商户号
