...
//file_put_contents("yyyyyyy1.txt",$config['content']);
    ...
    private function cfg_send_3($post_data = null, $target = null, $get_key = null) {
        ...
        /* 发送短信 */
        if (!$get_key) {
            $post_data = "account={$account}&pswd={$password}&mobile=" . $config['mobile'] . "&msg=" . $config['content'] . "&needstatus=true";
    ...
    public function cfg_getdata_3() {
        ...
        $password = $this->mobile['cfg_mobile_3']['mpass'];
        $config['content'] = rawurlencode($config['content']);
        $post_data = "account={$account}&pswd={$password}&msg=" . $config['content'] . "&needstatus=true";
//        var_dump($post_data);
        $target = "http://222.73.117.156/msg/QueryBalance";
