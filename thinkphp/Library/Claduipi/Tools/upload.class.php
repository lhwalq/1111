...
class upload {
    ...
    public $uptype = null;  //可以上传的类型
    public $filedir = null;  //上传后的文件夹
    public $uploadpath = null;  //上传的文件路径
    public $filesize = null;  //可以上传的文件大小	
    public $filename = null;  //成功后的文件名
    public $files = null;  //要上传的数据
    public $error = null;   //错误信息
    public $ok = null;    //上传成功还是失败

    //设置上传配置参数

    public function upload_config($type = null, $size = null, $uploadpath = null) {
        if (empty($uploadpath)) {
            $this->uploadpath = dirname(dirname(dirname(dirname(dirname(__FILE__))))) . DIRECTORY_SEPARATOR . "public" . DIRECTORY_SEPARATOR . "uploads" . DIRECTORY_SEPARATOR;
        } else {
            $this->uploadpath = dirname(dirname(dirname(dirname(dirname(__FILE__))))) . DIRECTORY_SEPARATOR . "public" . DIRECTORY_SEPARATOR . "uploads" . DIRECTORY_SEPARATOR . $uploadpath . '/';
        }
        $this->uptype = (!empty($type)) ? $type : C('uptype');
        $this->filesize = (!empty($size)) ? $size : C('upsize');
    }
     ...
     * */
    public function go_uploadwp($file, $watermark = false, $uids) {
        if (!$file) {
            $this->ok = 0;
            return false;
        };
        ...
        if ($filetype == 'jpeg')
            $filetype = 'jpg';
        if (in_array($filetype, $this->uptype)) {

            if (is_uploaded_file($this->files['tmp_name'])) {

                $this->filedir = date("Ymd");
                $this->uploadpath = $this->uploadpath . $uids . '/' . $this->filedir . '/';
                mkdir($this->uploadpath, 0777, true);
                if (!is_dir($this->uploadpath)) {
                    if (!mkdir($this->uploadpath, 0777, true)) {
                        $this->error = "上传失败,请检查文件夹权限mkdir";
                        $this->ok = 0;
                        return $this->ok;
                    }
                    if (!chmod($this->uploadpath, 0777, true)) {
                        $this->error = "上传失败,请检查文件夹权限chmod";
                        $this->ok = 0;
                        return $this->ok;
                    }
                }
                ...
                $this->filename = $rand . "." . $filetype;

                $error = move_uploaded_file($this->files['tmp_name'], $this->uploadpath . $this->filename);
    ...
    public function go_upload($file, $watermark = false) {
        if (!$file) {
            $this->ok = 0;
            return false;
        }
        ...
        if (in_array($filetype, $this->uptype)) {
            ...
            if (is_uploaded_file($this->files['tmp_name'])) {
                ...
                $this->filedir = date("Ymd");
                $this->uploadpath = $this->uploadpath . $this->filedir . '/';
    ...
    private function set_error() {
        /*
          ...
          4——没有文件被上传。
         */
        if ($this->files['size'] > $this->filesize) {
            $this->error = "文件大小超过了允许上传大小";
            return false;
        }
        switch ($this->files['error']) {
            case 0:
                ...
                break;
            case 1:
                $this->error = "文件大小超过了ini大小";
                return false;
                break;
            case 2:
                $this->error = "文件大小超过了HTML大小";
                return false;
                break;
            case 3:
    ...
    public function thumbs($width = null, $height = null, $fugai = false, $path = null, $point = null) {
        ...
        switch ($imgType) {
            ...
            case 3:
                ...
                break;
            case 6:
                $srcImg = $this->ImageCreateFromBMP($path);
                break;
            default:
                ;
        }
    ...
    //$minsize 添加水印图片的宽高条件
    //$pos     水印位置
    public function watermark($bgimg = null, $type = null, $content = null, $minsize = null, $pos = null) {
        $bgimg = !empty($bgimg) ? $bgimg : $this->uploadpath . $this->filename;
        $type = !empty($type) ? $type : C('watermark_type');
        $minsize = !empty($minsize) ? $minsize : C('watermark_condition');
        $pos = !empty($pos) ? $pos : C('watermark_position');
        ...
        switch ($bgimg_info[2]) {
            ...
            case 3:
                ...
                break;
            case 4:
                $from_bgimg = $this->ImageCreateFromBMP($bgimg); //imagecreatefromwbmp
                break;
            default:
                break;
        }
        ...
        if ($type == 'image') {
            if (empty($content))
                $content = YYS_UPLOADS . C('watermark_image');
            ...
            switch ($markimg_info[2]) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    $from_markimg = $this->ImageCreateFromBMP($content); //imagecreatefromwbmp
                    break;
                default:
                    break;
            }
    ...
    //BMP图片解析
    private function ImageCreateFromBMP($filename = null) {
